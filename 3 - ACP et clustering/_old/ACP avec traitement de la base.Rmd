...
df[, densite := nb_personnes_menage/aire]
...
```{r}
df_cluster_com = df_5_cluster_2017_com[, c("COM", "cluster_com")]
```
...
Export des communes avec Paris
```{r}
# communes de Paris
paris_com <- as.character(75101:75120)

# 1. clusters présents à Paris
clusters_paris <- unique(
  df_cluster_COM_IRIS_2017[
    COM %in% paris_com,
    cluster
  ]
)

# 2. communes hors Paris ayant des IRIS dans ces clusters
communes_cluster_paris <- unique(
  df_cluster_COM_IRIS_2017[
    !(COM %in% paris_com) & cluster %in% clusters_paris,
    COM
  ]
)

# 3. transformer en data.table pour export
communes_cluster_paris_dt <- data.table(COM = communes_cluster_paris)

# 4. export CSV
fwrite(
  communes_cluster_paris_dt,
  "communes_cluster_paris.csv"
)

# petit contrôle
cat("Nombre de communes retenues :", length(communes_cluster_paris), "\n")
```
